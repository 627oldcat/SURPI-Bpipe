...
then
	...
	START1=$(date +%s)
	cutadapt_quality.csh $basef.modheader.fastq S $length_cutoff $keep_short_reads $adapter_set $temporary_files_directory
	mv $basef.modheader.cutadapt.fastq $basef.cutadapt.fastq
else
	START1=$(date +%s)
	cutadapt_quality.csh $inputfile S $length_cutoff $keep_short_reads $adapter_set $temporary_files_directory
fi
