...
######################## RUN SNAP ##########################
	...
	if [ $counter -eq 0 ]
	then
		cp $basef.fastq /dev/shm
    		/usr/bin/time -o $basef.snap.log snap-aligner single $snap_index /dev/shm/$basef.fastq -o $basef.tmp.sam -t $cores -x -f -h 250 -d $SNAP_d_cutoff -n 25 > $basef.time.log
		cp $basef.tmp.sam temp.sam
  		rm -f /dev/shm/$basef.fastq
	else
                cp $basef.tmp.fastq /dev/shm
		/usr/bin/time -o $basef.snap.log snap-aligner single $snap_index /dev/shm/$basef.tmp.fastq -o $basef.tmp.sam -t $cores -x -f -h 250 -d $SNAP_d_cutoff -n 25  > $basef.time.log
  		rm -f /dev/shm/$basef.tmp.fastq
	fi
